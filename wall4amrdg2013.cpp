...
//hr 参考点到墙的距离，hg壁面到ghost点的距离
//使用的时候一定搞清楚法向方向以及和riemann求解器法向速度是否一样
void reflection(double hr, double hg, double vnsolid, double U[4], const PXYZ *nxyz, JBBL *jbugst)
{
    double tx,ty,nx,ny;
    nx=nxyz->x;
    ny=nxyz->y;
...
void getGhostValueRiemann(OctCell * ghostcell)
{
    ...
    double pwh1, vnsolid,pw1;	
    double  hc,hg;
    double W00[4],U00[4],U01[4];
    double xs,ys,xcs,ycs,dxs,dys;
    double vac;
    ...
    getsmtrctAB(hc,&ghostpt, &plsA, &plsB,
    ...
    if(Nr==0)
        getGridIndex3(ghostcell, &symtrcpt, pincell);
    else
        getGridIndex(ghostcell, &symtrcpt, pincell);
  ...
  /*  for(int tm=0;tm<4;++tm){
        W00[tm]=pincell->dof[tm][0];
    }
    shbl2jbbl(W00,U01);
*/

    for(int itp=0;itp<4;++itp){
        W00[itp]=n_approx_sol(xs,ys,xcs,ycs,dxs,dys,pincell->dof[itp],nDOF);
    }

    shbl2jbbl(W00,U00);
    vac=4.0*sqrt(GAMMA*U00[3]/U00[0])/GAM11
        -(U00[1]*nml1.x+U00[2]*nml1.y);
    vac=-1.0;

    if(vac>0.0){
        if(qvlvRie == 0)
            reflectionRie(hc,hg,0.0,U00, &nml1, &jbgst1);
        else
            reflectionRieQVLV(hc,hg,0.0,U00, &nml1,&jiaodian, &jbgst1);
    }
    else{
        reflection(hc,hg,0.0,U00, &nml1, &jbgst1);
    }
...
void wall_treatment()
{
    ...
    {
        if(b_ARS)
            getGhostValueRiemann(current->cell);
        else
